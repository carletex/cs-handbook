...
class MinHeap {
  ...
  public function push($x) {
    ...
    $idx = $this->size - 1;
    $parent = ($idx - 1) / 2;
    // Push the node up until the parent is smaller.
    ...
    while ($idx < $this->size) {
      ...
      if ($left < $this->size && $right < $this->size) {
        ...
        if ($this->arr[$left] < $this->arr[$right] &&
          ...
          // If left child is smaller than right child and
          // current node.
          $this->swap($left, $idx);
          $idx = $left;
        } elseif ($this->arr[$right] <= $this->arr[$left] &&
          $this->arr[$right] < $this->arr[$idx]) {
          // If right child is smaller or equal than left child
          // and current node.
          $this->swap($right, $idx);
          $idx = $right;
        } else {
  ...
  public function pop() {
    ...
    $this->size--;
    // Push the root down until parent is smaller than
    // children
    $this->bubbleDown(0);
    return $ret;
  }
